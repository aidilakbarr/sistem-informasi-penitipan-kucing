      ...
      <div className="space-y-2">
        {segments.map((s) => (
          <div key={s.label} className="flex items-center gap-2 text-xs">
            ...
            <span className="font-bold text-stone-900 ml-auto pl-3">
              {s.value}
            </span>
          </div>
        ))}
...
export function AdminReportsPage() {
  ...
  const activeBookings = MOCK_BOOKINGS.filter((b) =>
    ["ONGOING", "CHECKED_IN"].includes(b.status),
  );

  const bookingSegments = [
    {
      label: "Selesai",
      value: MOCK_BOOKINGS.filter((b) => b.status === "COMPLETED").length,
      color: "bg-stone-400",
    },
    {
      label: "Berlangsung",
      value: MOCK_BOOKINGS.filter((b) => b.status === "ONGOING").length,
      color: "bg-green-500",
    },
    {
      label: "Pending",
      value: MOCK_BOOKINGS.filter((b) => b.status === "PENDING").length,
      color: "bg-amber-500",
    },
    {
      label: "Disetujui",
      value: MOCK_BOOKINGS.filter((b) => b.status === "APPROVED").length,
      color: "bg-blue-500",
    },
    {
      label: "Dibatalkan",
      value: MOCK_BOOKINGS.filter((b) => b.status === "CANCELLED").length,
      color: "bg-red-400",
    },
  ].filter((s) => s.value > 0);
  ...
  const serviceData = [
    {
      label: "Regular",
      count: MOCK_BOOKINGS.filter((b) => b.services.includes("REGULAR")).length,
    },
    {
      label: "VIP",
      count: MOCK_BOOKINGS.filter((b) => b.services.includes("VIP")).length,
    },
    {
      label: "Grooming",
      count: MOCK_BOOKINGS.filter((b) => b.services.includes("GROOMING"))
        .length,
    },
    {
      label: "Antar-Jemput",
      count: MOCK_BOOKINGS.filter((b) => b.services.includes("ANTAR_JEMPUT"))
        .length,
    },
  ];

  const weeklyData = [
    { day: "Sen", revenue: 1_200_000 },
    { day: "Sel", revenue: 1_875_000 },
    { day: "Rab", revenue: 950_000 },
    { day: "Kam", revenue: 2_100_000 },
    { day: "Jum", revenue: 1_650_000 },
    { day: "Sab", revenue: 2_450_000 },
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      <PageHeader
        ...
        subtitle="Ringkasan performa bisnis AnZ Pet Care"
        action={
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          icon="✅"
          iconBg="bg-stone-50"
          sub={`${Math.round((completedBookings.length / MOCK_BOOKINGS.length) * 100)}% completion rate`}
        />
        <StatCard
          label="Booking Aktif"
            ...
            <div className="px-6 pb-4">
              ...
              <div className="mt-4 pt-4 border-t border-stone-100 flex justify-between text-sm">
                ...
                <div className="text-right">
                  ...
                  <p className="font-black text-stone-900">
                    {formatCurrency(Math.round(totalRevenue / 6))}
                  </p>
                </div>
              </div>
            </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        {/* Service popularity */}
        <SectionCard
          title="Popularitas Layanan"
          subtitle="Jumlah booking per jenis layanan"
        >
              ...
              <div key={s.label} className="flex items-center gap-3">
                ...
                <div className="flex-1 bg-stone-100 rounded-full h-2 overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${(s.count / Math.max(...serviceData.map((x) => x.count))) * 100}%`,
                    }}
                  />
                </div>
                <span className="text-xs font-bold text-stone-700 w-6 text-right">
                  {s.count}
                </span>
              </div>
            ))}
                ...
                <Tr key={d.month}>
                  ...
                  <Td>
                    {i === 0 ? (
                      ...
                      <span
                        className={`text-xs font-bold ${positive ? "text-green-600" : "text-red-600"}`}
                      >
        ...
        >
          {[
            ...
            <Tr key={c.name}>
              ...
              <Td>
                <span
                  className={`text-xs font-semibold px-2 py-0.5 rounded-full ${c.active ? "bg-green-50 text-green-700" : "bg-stone-100 text-stone-500"}`}
                >
                  {c.active ? "Aktif" : "—"}
                </span>
              </Td>
